...
matched AS (
    SELECT h.ClinicID, h.typepatients, h.Sex, h.DaBirth, h.VLDate, h.VLValue, MIN(e.EACDate) AS eac3_date
    FROM latest_high_vl h
    INNER JOIN eac_visits e ON e.ClinicID = h.ClinicID AND e.EACSession = 3
...
SELECT main.ClinicID AS clinicid, art.ART AS art_number, m.Sex AS sex,
    ...
    TIMESTAMPDIFF(YEAR, m.DaBirth, :EndDate) AS age, m.VLDate AS high_vl_date, m.VLValue AS high_vl_value, m.eac3_date
FROM matched m
...
SELECT main.ClinicID, art.ART, m.Sex, CASE WHEN m.Sex = 0 THEN 'Female' ELSE 'Male' END,
    ...
    TIMESTAMPDIFF(YEAR, m.DaBirth, :EndDate), m.VLDate, m.VLValue, m.eac3_date
FROM matched m
