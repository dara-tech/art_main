-- 10.13 VL follow-up within 6 months after last EAC — patient detail
WITH adult_vl AS (
...
last_eac AS (
    SELECT h.ClinicID, h.typepatients, h.Sex, h.DaBirth, h.VLDate, h.VLValue, MAX(e.EACDate) AS LastEACDate
...
followup_ranked AS (
    ...
    WHERE pt.HIVLoad IS NOT NULL AND pt.HIVLoad <> ''
      AND pt.Dat > e.LastEACDate AND pt.Dat <= DATE_ADD(e.LastEACDate, INTERVAL 6 MONTH)
),
followup_met AS (
    SELECT * FROM followup_ranked WHERE rn = 1
)
...
SELECT main.ClinicID AS clinicid, art.ART AS art_number, f.Sex AS sex,
    ...
    f.LastEACDate AS last_eac_date, f.FollowupDate AS followup_vl_date, f.FollowupValue AS followup_vl_value
FROM followup_met f
...
SELECT main.ClinicID, art.ART, f.Sex, CASE WHEN f.Sex = 0 THEN 'Female' ELSE 'Male' END,
    ...
    TIMESTAMPDIFF(YEAR, f.DaBirth, :EndDate), f.VLDate, f.VLValue, f.LastEACDate, f.FollowupDate, f.FollowupValue
FROM followup_met f
