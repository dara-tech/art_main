-- Indicator 11.5.1: Number of active ART patients who started ART >= 6 months ago
WITH tblvisit AS (
    SELECT clinicid
    FROM (
        SELECT
...
SELECT
    '11.5.1. Started ART > 6 months' AS Indicator,
...
WHERE e.status IS NULL
  ...
  AND TIMESTAMPDIFF(MONTH, a.DaArt, :EndDate) >= 6;
