-- 10.13: High VL + EAC (sessions 1-3) with follow-up VL within 6 months after last EAC session
WITH adult_vl AS (
    SELECT
        '15+' AS typepatients,
...
last_eac AS (
    ...
    GROUP BY h.ClinicID, h.typepatients, h.Sex, h.VLDate
),
followup_ranked AS (
    SELECT
        e.ClinicID,
    ...
    WHERE pt.HIVLoad IS NOT NULL
      AND pt.HIVLoad <> ''
      AND pt.Dat > e.LastEACDate
      AND pt.Dat <= DATE_ADD(e.LastEACDate, INTERVAL 6 MONTH)
),
followup_met AS (
