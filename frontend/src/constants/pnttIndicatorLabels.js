/**
 * Khmer labels for PNTT indicators (mirrors backend/src/constants/pnttIndicatorLabels.js)
 */
...
export function labelKhForPnttScript(scriptId) {
  const key = normScriptId(scriptId);
...
/** Lookup by full visualize id `pntt:scriptId` */
export function labelKhForPnttIndicatorId(indicatorId) {
  const raw = String(indicatorId || '');
  if (!raw.startsWith('pntt:')) return null;
  return labelKhForPnttScript(raw.slice(5));
}
