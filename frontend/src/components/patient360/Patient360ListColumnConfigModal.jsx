...
export default function Patient360ListColumnConfigModal({
  ...
  return createPortal(
    ...
    >
      <div
        className="flex max-h-[88vh] w-full max-w-lg flex-col overflow-hidden bg-card shadow-2xl shadow-black/15"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="flex items-center justify-between border-b border-border/80 bg-muted/35 px-5 py-4">
          <div className="flex items-center gap-2">
            <RiLayoutColumnLine className="size-5 text-primary" aria-hidden />
            <div>
              <h2 id="p360-column-config-title" className="text-base font-semibold text-foreground">
                {P360_KH.list.columnConfigTitle}
              </h2>
              <p className="mt-0.5 text-xs text-muted-foreground">{P360_KH.list.columnConfigHint}</p>
            </div>
          </div>
          <button
            type="button"
